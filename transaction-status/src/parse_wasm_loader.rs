...
mod test {
    ...
    #[test]
    fn test_parse_wasm_loader_instructions() {
        ...
        let instruction = domichain_sdk::loader_instruction::write(
            &account_pubkey,
            &program_id,
            offset,
            bytes.clone(),
        ...
        let instruction = domichain_sdk::loader_instruction::finalize(&account_pubkey, &program_id);
        let mut message = Message::new(&[instruction], Some(&fee_payer));
        assert_eq!(
            parse_wasm_loader(
                &message.instructions[0],
    ...
    #[test]
    fn test_parse_wasm_upgradeable_loader_create_buffer_ix() {
        let max_data_len = 54321;

        let payer_address = Pubkey::new_unique();
        let buffer_address = Pubkey::new_unique();
        let authority_address = Pubkey::new_unique();
        let instructions = wasm_loader_upgradeable::create_buffer(
            &payer_address,
            &buffer_address,
            &authority_address,
            55,
        ...
        assert!(parse_wasm_upgradeable_loader(
            &message.instructions[1],
    ...
    #[test]
    fn test_parse_wasm_upgradeable_loader_write_ix() {
        let offset = 4242;
        let bytes = vec![8; 99];
    ...
    #[test]
    fn test_parse_wasm_upgradeable_loader_deploy_ix() {
        let max_data_len = 54321;
        ...
        .0;
        let instructions = wasm_loader_upgradeable::deploy_with_max_program_len(
            &payer_address,
            &program_address,
            &buffer_address,
    ...
    #[test]
    fn test_parse_wasm_upgradeable_loader_upgrade_ix() {
        let program_address = Pubkey::new_unique();
        let buffer_address = Pubkey::new_unique();
        let authority_address = Pubkey::new_unique();
        ...
        .0;
        let instruction = wasm_loader_upgradeable::upgrade(
            &program_address,
            &buffer_address,
            &authority_address,
            &spill_address,
    ...
    #[test]
    fn test_parse_wasm_upgradeable_loader_set_buffer_authority_ix() {
        let buffer_address = Pubkey::new_unique();
        let current_authority_address = Pubkey::new_unique();
        let new_authority_address = Pubkey::new_unique();
        let instruction = wasm_loader_upgradeable::set_buffer_authority(
            &buffer_address,
    ...
    #[test]
    fn test_parse_wasm_upgradeable_loader_set_buffer_authority_checked_ix() {
        let buffer_address = Pubkey::new_unique();
        let current_authority_address = Pubkey::new_unique();
        let new_authority_address = Pubkey::new_unique();
        let instruction = wasm_loader_upgradeable::set_buffer_authority_checked(
            &buffer_address,
            &current_authority_address,
            &new_authority_address,
        );
    ...
    #[test]
    fn test_parse_wasm_upgradeable_loader_set_upgrade_authority_ix() {
        let program_address = Pubkey::new_unique();
        let current_authority_address = Pubkey::new_unique();
        let new_authority_address = Pubkey::new_unique();
        ...
        let instruction = wasm_loader_upgradeable::set_upgrade_authority(
            &program_address,
            &current_authority_address,
    ...
    #[test]
    fn test_parse_wasm_upgradeable_loader_set_upgrade_authority_checked_ix() {
        let program_address = Pubkey::new_unique();
        let current_authority_address = Pubkey::new_unique();
        let new_authority_address = Pubkey::new_unique();
        let (programdata_address, _) = Pubkey::find_program_address(
        ...
        let instruction = wasm_loader_upgradeable::set_upgrade_authority_checked(
            &program_address,
            &current_authority_address,
            &new_authority_address,
        );
    ...
    #[test]
    fn test_parse_wasm_upgradeable_loader_close_buffer_ix() {
        let close_address = Pubkey::new_unique();
        let recipient_address = Pubkey::new_unique();
        let authority_address = Pubkey::new_unique();
        let instruction =
            wasm_loader_upgradeable::close(&close_address, &recipient_address, &authority_address);
        let mut message = Message::new(&[instruction], None);
        assert_eq!(
            parse_wasm_upgradeable_loader(
                &message.instructions[0],
    ...
    #[test]
    fn test_parse_wasm_upgradeable_loader_close_program_ix() {
        let close_address = Pubkey::new_unique();
        let recipient_address = Pubkey::new_unique();
        let authority_address = Pubkey::new_unique();
        let program_address = Pubkey::new_unique();
        let instruction = wasm_loader_upgradeable::close_any(
            &close_address,
            &recipient_address,
