...
//! [`ecrecover`] precompile. For that Domichain provides the [`secp256k1_recover`]
//! syscall.
//!
//! [secp256k1]: https://en.bitcoin.it/wiki/Secp256k1
//! [`secp256k1_program`]: domichain_program::secp256k1_program
//! [`secp256k1_recover`]: domichain_program::secp256k1_recover
...
//!
//! The `domichain_sdk` crate provides few APIs for building the instructions and
//! transactions necessary for properly using the secp256k1 native program.
//! Many steps must be done manually.
//!
//! The `domichain_program` crate provides no APIs to assist in interpreting
...
//! and callers should ensure they use the same version of `bincode` as the Domichain SDK.
//! This data structure is not provided to Domichain programs,
...
//! to parse the secp256k1 instruction data from within a Domichain program.
//!
//! ```no_run
//! mod secp256k1_defs {
...
//!
//! This example has two components: a Domichain program, and an RPC client that
//! sends a transaction to call it. The RPC client will sign a single message,
//! and the Domichain program will introspect the secp256k1 instruction to verify
//! that the signer matches a known authorized public key.
//!
//! The Domichain program. Note that it uses `libsecp256k1` version 0.7.0 to parse
...
//!     // Domichain does not do this itself.
...
//! containing many signatures, and a Domichain program that parses them all. This
...
//! transaction. In the _previous_ example, the Domichain program asserted that all
//! signatures, messages, and addresses were stored in the instruction at 0. In
//! this next example the Domichain program supports signatures, messages, and
...
pub fn new_secp256k1_instruction(
    ...
    Instruction {
        program_id: domichain_sdk::secp256k1_program::id(),
        accounts: vec![],
...
///
/// `feature_set` is the set of active Domichain features. It is used to enable or
