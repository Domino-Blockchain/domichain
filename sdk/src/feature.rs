...
pub use domichain_program::feature::*;
...
pub fn create_account(feature: &Feature, satomis: u64) -> AccountSharedData {
    let data_len = Feature::size_of().max(bincode::serialized_size(feature).unwrap() as usize);
    let mut account = AccountSharedData::new(satomis, data_len, &id());
