...
fn find_installed_platform_tools() -> Vec<String> {
    ...
    let domichain = home_dir.join(".cache").join("domichain");
    let package = "platform-tools";
    std::fs::read_dir(domichain)
        .unwrap()
...
fn get_latest_platform_tools_version() -> Result<String, String> {
    let url = "https://github.com/domichain-labs/platform-tools/releases/latest";
...
// check whether custom domichain toolchain is linked, and link it if it is not.
fn link_domichain_toolchain(config: &Config) {
    ...
    for line in rustup_output.lines() {
        if line.starts_with("domichain") {
            let mut it = line.split_whitespace();
            let _ = it.next();
            let path = it.next();
            if path.unwrap() != toolchain_path.to_str().unwrap() {
                let rustup_args = vec!["toolchain", "uninstall", "domichain"];
                let output = spawn(
                    &rustup,
                    rustup_args,
                    config.generate_child_script_on_failure,
...
fn build_domichain_package(
    ...
    info!("Domichain SDK: {}", config.sbf_sdk.display());
    ...
    install_if_missing(
        config,
        package,
        "https://github.com/domichain-labs/platform-tools/releases/download",
    ...
    link_domichain_toolchain(config);
    ...
    // other than the one linked in Domichain toolchain. We have to prevent
    // this by removing RUSTC from the child process environment.
    if env::var("RUSTC").is_ok() {
        warn!(
            "Removed RUSTC from cargo environment, because it overrides +domichain cargo command line option."
        );
        env::remove_var("RUSTC")
    }
    let cargo_target = "CARGO_TARGET_SBF_DOMICHAIN_DOMICHAIN_RUSTFLAGS";
...
fn build_domichain(config: Config, manifest_path: Option<PathBuf>) {
    ...
    if let Some(root_package) = metadata.root_package() {
        if !config.workspace {
            build_domichain_package(&config, metadata.target_directory.as_ref(), root_package);
            return;
        }
    }
    ...
    for package in all_sbf_packages {
        build_domichain_package(&config, metadata.target_directory.as_ref(), package);
    }
}
...
fn main() {
    ...
    let matches = clap::Command::new(crate_name!())
        ...
        .arg(
            Arg::new("workspace")
                ...
                .takes_value(false)
                .alias("all")
                .help("Build all Domichain packages in the workspace"),
        )
        .arg(
            Arg::new("jobs")
                .short('j')
    ...
    let config = Config {
        ...
        sbf_sdk: fs::canonicalize(&sbf_sdk).unwrap_or_else(|err| {
            error!(
                "Domichain SDK path does not exist: {}: {}",
                sbf_sdk.display(),
                err
            );
            exit(1);
    ...
    build_domichain(config, manifest_path);
}
