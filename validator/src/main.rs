...
use {
    ...
    domichain_clap_utils::input_parsers::{keypair_of, keypairs_of, pubkey_of, value_of},
    domichain_core::{
        ...
        validator::{
            ...
            ValidatorConfig, ValidatorStartProgress, DEFAULT_TOTAL_WEIGHT,
        },
    },
    domichain_gossip::{cluster_info::Node, legacy_contact_info::LegacyContactInfo as ContactInfo},
    ...
    domichain_rpc_client_api::config::RpcLeaderScheduleConfig,
    domichain_runtime::{
        accounts_db::{
...
pub fn main() {
        ...
        ("set-public-address", Some(subcommand_matches)) => {
            let parse_arg_addr = |arg_name: &str, arg_long: &str| -> Option<SocketAddr> {
                subcommand_matches.value_of(arg_name).map(|host_port| {
                        domichain_net_utils::parse_host_port(host_port).unwrap_or_else(|err| {
    ...
    info!("{} {}", crate_name!(), domichain_version);
    ...
    let mut validator_config = ValidatorConfig {
        ...
        total_weight: value_of(&matches, "total_weight").unwrap_or(DEFAULT_TOTAL_WEIGHT),
    ...
    if !matches.is_present("no_os_network_limits_test") {
        if SystemMonitorService::check_os_network_limits() {
            ...
            eprintln!("OS network limit test failed. See: https://docs.domichain.com/running-validator/validator-start#system-tuning");
            exit(1);
        }
    }
    ...
    let public_tpu_addr = matches.value_of("public_tpu_addr").map(|public_tpu_addr| {
        domichain_net_utils::parse_host_port(public_tpu_addr).unwrap_or_else(|err| {
    ...
    let public_tpu_forwards_addr =
        matches
            ...
            .map(|public_tpu_forwards_addr| {
                domichain_net_utils::parse_host_port(public_tpu_forwards_addr).unwrap_or_else(|err| {
    ...
    domichain_metrics::set_panic_hook("validator", Some(String::from(domichain_version)));
