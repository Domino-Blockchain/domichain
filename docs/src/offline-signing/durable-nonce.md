...
They are implemented as a Domichain Program, the mechanics of which can be read
...
```bash
domichain-keygen new -o nonce-keypair.json
domichain create-nonce-account nonce-keypair.json 1
```
...
> [Full usage documentation](../cli/usage.md#domichain-create-nonce-account)
...
```bash
domichain nonce nonce-keypair.json
```
...
> [Full usage documentation](../cli/usage.md#domichain-get-nonce)
...
```bash
domichain new-nonce nonce-keypair.json
```
...
> [Full usage documentation](../cli/usage.md#domichain-new-nonce)
...
```bash
domichain nonce-account nonce-keypair.json
```
...
> [Full usage documentation](../cli/usage.md#domichain-nonce-account)
...
```bash
domichain withdraw-from-nonce-account nonce-keypair.json ~/.config/domichain/id.json 0.5
```
...
> [Full usage documentation](../cli/usage.md#domichain-withdraw-from-nonce-account)
...
```bash
domichain authorize-nonce-account nonce-keypair.json nonce-authority.json
```
...
> [Full usage documentation](../cli/usage.md#domichain-authorize-nonce-account)
...
- [`pay`](../cli/usage.md#domichain-pay)
- [`delegate-stake`](../cli/usage.md#domichain-delegate-stake)
- [`deactivate-stake`](../cli/usage.md#domichain-deactivate-stake)
...
```bash
$ domichain-keygen new -o alice.json
$ domichain-keygen new -o nonce.json
$ domichain-keygen new -o bob.json
```
...
```bash
$ domichain airdrop -k alice.json 1
1 DOMI
```
...
```bash
$ domichain create-nonce-account -k alice.json nonce.json 0.1
...
```bash
$ domichain transfer -k alice.json --blockhash expiredDTaxfagttWjQweib42b6ZHADSx94Tw8gHx11 bob.json 0.01
[2020-01-02T18:48:28.462911000Z ERROR domichain_cli::cli] Io(Custom { kind: Other, error: "Transaction \"33gQQaoPc9jWePMvDAeyJpcnSPiGUAdtVg8zREWv4GiKjkcGNufgpcbFyRKRrA25NkgjZySEeKue5rawyeH5TzsV\" failed: None" })
...
```bash
$ domichain nonce-account nonce.json
balance: 0.1 DOMI
minimum balance required: 0.00136416 DOMI
...
```bash
$ domichain transfer -k alice.json --blockhash F7vmkY3DTaxfagttWjQweib42b6ZHADSx94Tw8gHx3W7 --nonce nonce.json bob.json 0.01
