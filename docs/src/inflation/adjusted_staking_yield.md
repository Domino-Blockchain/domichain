...
As seen in the plot above, the proportion of staked tokens increases with inflation issuance. Letting $SOL_s(t)$ and $DOMI_{\text{total}}(t)$ represent the amount of staked and total DOMI at time $t$ respectively:

$$
	P_s(t_2) = \frac{SOL_s(t_1) + DOMI_{\text{total}}(t_1)\times I(t_1)}{DOMI_{\text{total}}(t_1)\times (1 + I(t_1))}\\
$$
...
$$
\begin{aligned}
Y_{adj} &= \frac{\frac{SOL_s(t) + DOMI_{\text{total}}(t)\times I(t)}{DOMI_{\text{total}}(t)\times (1 + I(t))} - \frac{SOL_s(t)}{DOMI_{\text{total}}(t)} }{ \frac{SOL_s(t)}{DOMI_{\text{total}}(t)} }  \\
	&= \frac{ DOMI_{\text{total}}(t)\times (SOL_s(t) + DOMI_{\text{total}}(t)\times I(t)) }{ SOL_s(t)\times DOMI_{\text{total}}\times (1 + I(t)) } -1 \\
\end{aligned}
$$
