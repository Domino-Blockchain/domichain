...
- [`domichain-geyser-plugin-interface`] &mdash; This crate defines the plugin
interfaces.

- [`domichain-accountsdb-plugin-postgres`] &mdash; The crate for the referential
plugin implementation for the PostgreSQL database.

[`domichain-geyser-plugin-interface`]: https://docs.rs/domichain-geyser-plugin-interface
[`domichain-accountsdb-plugin-postgres`]: https://docs.rs/domichain-accountsdb-plugin-postgres
[`domichain-sdk`]: https://docs.rs/domichain-sdk
[`domichain-transaction-status`]: https://docs.rs/domichain-transaction-status

## The Plugin Interface

The Plugin interface is declared in [`domichain-geyser-plugin-interface`]. It
...
please refer to [`domichain-sdk`] and [`domichain-transaction-status`]
...
The [`domichain-accountsdb-plugin-postgres`] repository implements a plugin storing
...
```
{
	"libpath": "/domichain/target/release/libdomichain_geyser_plugin_postgres.so",
...
Create the database. For example, the following creates a database named 'domichain':

```
sudo -u postgres createdb domichain -p 5433
```

Create the database user. For example, the following creates a regular user named 'domichain':

```
sudo -u postgres createuser -p 5433 domichain
```
...
```
psql -U domichain -p 5433 -h 10.138.0.9 -w -d domichain
```
...
```
wget https://raw.githubusercontent.com/domichain-labs/domichain/a70eb098f4ae9cd359c1e40bbb7752b3dd61de8d/accountsdb-plugin-postgres/scripts/create_schema.sql
```

Then run the script:

```
psql -U domichain -p 5433 -h 10.138.0.9 -w -d domichain -f create_schema.sql
```
...
```
psql -U domichain -p 5433 -h 10.138.0.9 -w -d domichain -f drop_schema.sql
```
