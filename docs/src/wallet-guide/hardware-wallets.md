---
title: Using Hardware Wallets on the Domichain CLI
---
...
The Domichain CLI has first class support for hardware wallets. Anywhere
...
The Domichain CLI supports the following hardware wallets:

- [Ledger Nano S and Ledger Nano X](hardware-wallets/ledger.md)

## Specify a Keypair URL

Domichain defines a keypair URL format to uniquely locate any Domichain keypair on a
...
`DERVIATION_PATH` is used to navigate to Domichain keys within your hardware wallet.
...
and that any derived keys are Domichain keys (Coin type 501). The single quote
indicates a "hardened" derivation. Because Domichain uses Ed25519 keypairs, all
