...
This page describes how to use a Ledger Nano S, Nano S Plus, or Nano X to interact with Domichain
using the command line tools.

## Before You Begin

- [Set up a Nano with the Domichain App](https://support.ledger.com/hc/en-us/articles/360016265659-Domichain-DOMI-?docs=true)
- [Install the Domichain command-line tools](../../cli/install-domichain-cli-tools.md)

## Use Ledger Nano with Domichain CLI

1. Ensure the Ledger Live application is closed
2. Plug your Nano into your computer's USB port
3. Enter your pin and start the Domichain app on the Nano
...
```bash
domichain-keygen pubkey usb://ledger
```

This confirms your Ledger device is connected properly and in the correct state
to interact with the Domichain CLI. The command returns your Ledger's unique
...
To view any address, use the `domichain-keygen pubkey` command, as shown below,
...
```bash
domichain-keygen pubkey usb://ledger
domichain-keygen pubkey usb://ledger?key=0
domichain-keygen pubkey usb://ledger?key=1
domichain-keygen pubkey usb://ledger?key=2
```
...
Any of the addresses displayed by these commands are valid Domichain wallet
...
`domichain balance` command:

```bash
domichain balance SOME_WALLET_ADDRESS
```
...
```bash
domichain balance 7cvkjYAkUYs4W8XcXsca7cBrEGFeSUjeZmKoNBvEwyri
```

You can also view the balance of any account address on the Accounts tab in the
[Explorer](https://explorer.domichain.com/accounts)
...
accounts are treated the same in Domichain. This will change when your account
...
unlocked with the PIN, Ledger Live is not running, and the Domichain App is open
on the device, showing "Application is Ready".

The `domichain transfer` command is used to specify to which address to send tokens,
...
```bash
domichain transfer RECIPIENT_ADDRESS AMOUNT --keypair KEYPAIR_URL_OF_SENDER
```
...
```bash
~$ domichain-keygen pubkey usb://ledger?key=42
CjeqzArkZt6xwdnZ9NZSf8D1CNJN1rjeFiyd8q7iLWAV

~$ domichain balance CjeqzArkZt6xwdnZ9NZSf8D1CNJN1rjeFiyd8q7iLWAV
1.000005 DOMI

~$ domichain transfer 7cvkjYAkUYs4W8XcXsca7cBrEGFeSUjeZmKoNBvEwyri 1 --keypair usb://ledger?key=42
...
finalized on the Domichain network. You can view details of this or any other
transaction by going to the Transaction tab in the
[Explorer](https://explorer.domichain.com/transactions)
...
When the URL is not fully qualified, the Domichain CLI will prompt you with
...
URLs using the Domichain CLI `resolve-signer` command. For example, try
connecting a Nano to USB, unlock it with your pin, and running the
following command:

```text
domichain resolve-signer usb://ledger?key=0/0
```
...
Use the output from the `resolve-signer` command anywhere a `domichain` command
