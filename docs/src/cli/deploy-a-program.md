...
smart contracts elsewhere) with the Domichain tools.

To learn about developing and executing programs on Domichain, start with the
[intro to Domichain programs](developing/intro/programs.md) and then dig into the
details of [on-chain programs](developing/on-chain-programs/overview.md).

To deploy a program, use the Domichain tools to interact with the on-chain loader
to:
...
```bash
domichain program deploy <PROGRAM_FILEPATH>
```
...
```bash
domichain program deploy --program-id <KEYPAIR_FILEPATH> <PROGRAM_FILEPATH>
```
...
```bash
domichain program show <ACCOUNT_ADDRESS>
```
...
```bash
domichain program deploy --max-len 200000 <PROGRAM_FILEPATH>
```
...
`domichain-keygen recover` and the following 12-word seed phrase:
==================================================================================
...
```bash
domichain-keygen recover -o <KEYPAIR_PATH>
```
...
```bash
domichain program show --programs
domichain program show --buffers
```

To specify a different authority:

```bash
domichain program show --programs --buffer-authority <AURTHORITY_ADRESS>
domichain program show --buffers --buffer-authority <AURTHORITY_ADRESS>
```

To close a single account:

```bash
domichain program close <BADDRESS>
```

To close a single account and specify a different authority than the default:

```bash
domichain program close <ADDRESS> --buffer-authority <KEYPAIR_FILEPATH>
```

To close a single account and specify a different recipient than the default:

```bash
domichain program close <ADDRESS> --recipient <RECIPIENT_ADDRESS>
```

To close all the buffer accounts associated with the current authority:

```bash
domichain program close --buffers
```

To show all buffer accounts regardless of the authority

```bash
domichain program show --buffers --all
```
...
```bash
domichain program deploy --upgrade-authority <UPGRADE_AUTHORITY_SIGNER> <PROGRAM_FILEPATH>
```

Or after deployment and using the default keypair as the current authority:

```bash
domichain program set-upgrade-authority <PROGRAM_ADDRESS> --new-upgrade-authority <NEW_UPGRADE_AUTHORITY>
```

Or after deployment and specifying the current authority:

```bash
domichain program set-upgrade-authority <PROGRAM_ADDRESS> --upgrade-authority <UPGRADE_AUTHORITY_SIGNER> --new-upgrade-authority <NEW_UPGRADE_AUTHORITY>
```
...
```bash
domichain program deploy <PROGRAM_FILEPATH> --final
```

Or anytime after:

```bash
domichain program set-upgrade-authority <PROGRAM_ADDRESS> --final
```
...
```bash
domichain program dump <ACCOUNT_ADDRESS> <OUTPUT_FILEPATH>
```
...
```bash
$ domichain dump <ACCOUNT_ADDRESS> dump.so
...
```bash
domichain program write-buffer <PROGRAM_FILEPATH>
```

Buffer accounts support authorities like program accounts:

```bash
domichain program set-buffer-authority <BUFFER_ADDRESS> --new-buffer-authority <NEW_BUFFER_AUTHORITY>
```
...
```bash
domichain program deploy --program-id <PROGRAM_ADDRESS> --buffer <BUFFER_ADDRESS>
```
