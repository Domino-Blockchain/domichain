...
```bash
domichain catchup ~/validator-keypair.json
```
...
```bash
domichain-keygen new -o ~/validator-stake-keypair.json
```
...
```bash
domichain create-stake-account ~/validator-stake-keypair.json 1
```

and then delegating that stake to your validator:

```bash
domichain delegate-stake ~/validator-stake-keypair.json ~/vote-account-keypair.json
```
...
```bash
domichain delegate-stake ~/validator-stake-keypair.json ~/some-other-vote-account-keypair.json
```
...
The rewards satomis earned are split between your stake account and the vote
...
These vote transactions are paid for by satomis from a validator's identity
account.
...
- View your vote account:`domichain vote-account ~/vote-account-keypair.json` This displays the current state of all the votes the validator has submitted to the network.
- View your stake account, the delegation preference and details of your stake:`domichain stake-account ~/validator-stake-keypair.json`
- `domichain validators` displays the current active stake of all validators, including yours
- `domichain stake-history` shows the history of stake warming up and cooling down over recent epochs
- Look for log messages on your validator indicating your next leader slot: `[2019-09-27T20:16:00.319721164Z INFO domichain_core::replay_stage] <VALIDATOR_IDENTITY_PUBKEY> voted and reset PoH at tick height ####. My next leader slot is ####`
- Once your stake is warmed up, you will see a stake balance listed for your validator by running `domichain validators`

## Monitor Your Staked Validator

Confirm your validator becomes a [leader](../terminology.md#leader)

- After your validator is caught up, use the `domichain balance` command to monitor the earnings as your validator is selected as leader and collects transaction fees
...
```bash
domichain deactivate-stake ~/validator-stake-keypair.json
```
...
cli's `withdraw-stake` command to recover the previously staked satomis.
