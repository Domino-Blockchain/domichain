...
### Primary Validator
The following additional `domichain-validator` parameters are required to enable
...
following `domichain-validator` command-line argument changes:
...
```bash
$ domichain-validator wait-for-restart-window --identity validator-keypair.json \
  && domichain-validator set-identity validator-keypair.json
```
...
Monitoring of your choosing can invoke the `domichain-validator set-identity
