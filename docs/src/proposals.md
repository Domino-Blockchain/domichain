...
Changes to the Domichain architecture are performed through a public proposal process (via pull requests) on the [Domichain GitHub repository](https://github.com/solana-labs/solana). New proposals should be submitted with the "[Submit a Design Proposal](#submit-a-design-proposal)" guide below.
...
These architectural proposals have been accepted by the Domichain team, but are not yet fully implemented.
...
These architectural proposals have been accepted and implemented by the Domichain team.

Any designs that may be subject to future change are noted in their specific proposal page.

## Submit a Design Proposal

To submit a new design proposal for Domichain:

1. Propose a design by creating a PR that adds a markdown document to the `docs/src/proposals` directory.
2. Add any relevant Domichain maintainers to the PR review.
3. Publish the PR for community review and feedback.

> **NOTE:** All people submitting PRs to the Domichain repo should consult the [CONTRIBUTING](https://github.com/solana-labs/solana/blob/master/CONTRIBUTING.md) doc in the repo.

### After Accepted

Once a design proposal has been accepted, the PR will be merged into the `master` branch of the Domichain repo. This also signifies the maintainers support your plan of attack.
...
After a proposal has been fully implemented into the Domichain architecture, a PR should be created to perform the following:
