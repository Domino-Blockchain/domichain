...
A front-end application that interacts with a Domichain cluster.
...
The Domichain program that owns and loads [BPF](developing/on-chain-programs/overview#berkeley-packet-filter-bpf) smart contract programs, allowing the program to interface with the runtime.

## client

A computer program that accesses the Domichain server network [cluster](#cluster).
...
An increase in token supply over time used to fund rewards for validation and to fund continued development of Domichain.
...
Domichain's parallel smart contracts run-time.
