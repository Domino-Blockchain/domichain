...
use {
    ...
    domichain_client::rpc_request::MAX_MULTIPLE_ACCOUNTS,
    domichain_core::{
...
impl TestValidatorGenesis {
    ...
    pub fn add_accounts_from_json_files(
        ...
        for account in accounts {
            let account_path = match domichain_program_test::find_file(account.filename) {
                Some(path) => path,
    ...
    pub fn add_program(&mut self, program_name: &str, program_id: Pubkey) -> &mut Self {
        let program_path = domichain_program_test::find_file(&format!("{program_name}.so"))
...
impl TestValidator {
    ...
    fn initialize_ledger(
        ...
        for upgradeable_program in &config.upgradeable_programs {
            let data = domichain_program_test::read_file(&upgradeable_program.program_path);
