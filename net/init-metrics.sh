...
Creates a testnet dev metrics database
  ...
  -e              Assume database already exists and DOMICHAIN_METRICS_CONFIG is
...
host="https://internal-metrics.domichain.com:8086"
...
if $useEnv; then
  [[ -n $DOMICHAIN_METRICS_CONFIG ]] ||
    usage "DOMICHAIN_METRICS_CONFIG is not defined in the environment"
else
  username=$1
  ...
  DOMICHAIN_METRICS_CONFIG="host=$host,db=$netBasename,u=scratch_writer,p=topsecret"
fi

echo "export DOMICHAIN_METRICS_CONFIG=\"$DOMICHAIN_METRICS_CONFIG\"" >> "$configFile"
