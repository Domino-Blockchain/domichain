...
declare -r DOMICHAIN_LOCK_FILE="/home/domichain/.domichain.lock"
...
colo_instance_run() {
  ...
  declare OUT
  set +e
  OUT=$(ssh -l domichain -o "StrictHostKeyChecking=no" -o "ConnectTimeout=3" -n "${IP}" "${CMD}" 2>&1)
  declare RC=$?
  set -e
  while read -r LINE; do
...
colo_whoami() {
  ...
  done < <(colo_instance_run_foreach "[ -n \"\${DOMICHAIN_USER}\" ] && echo -e \"\${DOMICHAIN_USER}\\x1fEOL\"")
  echo "${ME}"
}

COLO_DOMICHAIN_USER=""
colo_get_domichain_user() {
  if [ -z "${COLO_DOMICHAIN_USER}" ]; then
    COLO_DOMICHAIN_USER=$(colo_whoami)
  fi
  echo "${COLO_DOMICHAIN_USER}"
}
...
__colo_node_status_script() {
  ...
  exec 2>/dev/null  # Suppress stderr as the next call to exec fails most of
                    # the time due to ${DOMICHAIN_LOCK_FILE} not existing and is running from a
                    # subshell where normal redirection doesn't work
  exec 9<"${DOMICHAIN_LOCK_FILE}" && flock -s 9 && . "${DOMICHAIN_LOCK_FILE}" && exec 9>&-
  echo -e "\${DOMICHAIN_LOCK_USER}\\x1f\${DOMICHAIN_LOCK_INSTANCENAME}\\x1f\${PREEMPTIBLE}\\x1fEOL"
...
SSH_AUTHORIZED_KEYS='$("${__colo_here}"/add-datacenter-domichain-user-authorized_keys.sh 2> /dev/null)'
