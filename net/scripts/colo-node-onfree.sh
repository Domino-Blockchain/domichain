...
if [[ -f "${DOMICHAIN_LOCK_FILE}" ]]; then
  ...
  . "${DOMICHAIN_LOCK_FILE}"
  if [[ "${DOMICHAIN_LOCK_USER}" = "${DOMICHAIN_USER}"  || -n "${FORCE_DELETE}" ]]; then
    ...
    git clean -qxdff
    rm -f /domichain-scratch/* /domichain-scratch/.[^.]*
...
EOAK
    ...
    echo "Invalid user: expected \"${DOMICHAIN_LOCK_USER}\" got \"${DOMICHAIN_USER}\"" 1>&2
  fi
  exec 9>&-
fi
${RC}
