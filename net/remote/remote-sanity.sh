...
local|tar|skip)
  ...
  domichain_cli=domichain
  domichain_gossip=domichain-gossip
  domichain_install=domichain-install
  ;;
*)
...
(
  set -x
  $domichain_cli --url http://"$sanityTargetIp":8899 validators
)
...
(
  ...
  $domichain_gossip --allow-private-addr spy --entrypoint "$sanityTargetIp:8001" \
...
if $installCheck && [[ -r update_manifest_keypair.json ]]; then
  echo "--- $sanityTargetIp: domichain-install test"

  (
    set -x
    rm -rf install-data-dir
    $domichain_install init \
      --no-modify-path \
      --data-dir install-data-dir \
      --url http://"$sanityTargetIp":8899 \

    $domichain_install info
  )
fi
