...
use {
    ...
    domichain_geyser_plugin_manager::{
    ...
    domichain_metrics::{datapoint_info, poh_timing_point::PohTimingSender},
    domichain_poh::{
    ...
    domichain_streamer::{socket::SocketAddrSpace, streamer::StakedNodes},
...
impl BlockstoreRootScan {
    fn new(config: &ValidatorConfig, blockstore: &Arc<Blockstore>, exit: &Arc<AtomicBool>) -> Self {
            ...
            && config.rpc_config.rpc_scan_and_fix_roots
        {
            ...
            Some(
                Builder::new()
                    .name("domiBStoreRtScan".to_string())
...
pub struct Validator {
    ...
    tpu: Tpu,
    tvu: Tvu,
    ip_echo_server: Option<domichain_net_utils::IpEchoServer>,
...
impl Validator {
    ...
    pub fn new(
        ...
        if rayon::ThreadPoolBuilder::new()
            .thread_name(|i| format!("domiRayonGlob{i:02}"))
            .build_global()
            .is_err()
        {
        ...
        if domichain_perf::perf_libs::api().is_some() {
        ...
        *start_progress.write().unwrap() = ValidatorStartProgress::StartingServices;
        ...
        let ip_echo_server = match node.sockets.ip_echo {
            None => None,
            Some(tcp_listener) => Some(domichain_net_utils::ip_echo_server(
                tcp_listener,
        ...
        *admin_rpc_service_post_init.write().unwrap() = Some(AdminRpcRequestMetadataPostInit {
        ...
        datapoint_info!(
            ...
            ("version", domichain_version::version!(), String)
        );
        ...
        if self.tower.is_none() {
            ...
            *self.start_progress.write().unwrap() = ValidatorStartProgress::LoadingLedger;
            ...
            if let Ok(Some(max_slot)) = self.blockstore.highest_slot() {
                ...
                let _ = Builder::new()
                    .name("domiRptLdgrStat".to_string())
                    .spawn(move || {
...
fn maybe_warp_slot(
    ...
    if let Some(warp_slot) = config.warp_slot {
        ...
        bank_forks.insert(Bank::warp_from_parent(
            ...
            &Pubkey::default(),
            warp_slot,
            domichain_runtime::accounts_db::CalcAccountsHashDataSource::Storages,
        ));
        bank_forks.set_root(
            warp_slot,
            accounts_background_request_sender,
...
mod tests {
    use {
        ...
        domichain_gossip::contact_info::{ContactInfo, LegacyContactInfo},
        ...
        domichain_tpu_client::tpu_client::{
    ...
    fn test_backup_and_clear_blockstore() {
        ...
        use {
            ...
            domichain_ledger::{blockstore, get_tmp_ledger_path},
        };
    ...
    fn test_poh_speed() {
        ...
        let poh_config = PohConfig {
            target_tick_duration: Duration::from_millis(domichain_sdk::clock::MS_PER_TICK),
            // make PoH rate really fast to cause the panic condition
            hashes_per_tick: Some(100 * domichain_sdk::clock::DEFAULT_HASHES_PER_TICK),
            ..PohConfig::default()
        };
