pub use domichain_ledger::blockstore_processor::CacheBlockMetaSender;
use {
...
impl CacheBlockMetaService {
    ...
    fn cache_block_meta(bank: Arc<Bank>, blockstore: &Arc<Blockstore>) {
        ...
        if let Err(e) = blockstore.cache_block_seed(bank.slot(), bank.block_seed()) {
            error!("cache_block_seed failed: slot {:?} {:?}", bank.slot(), e);
        }
    }
