...
if [[ -z "$url" ]]; then
  echo Provide complete URL, ex: "$0" http://api.devnet.domichain.com:8899
  exit 1
fi
domichain config set --url $url
...
domichain-keygen new -o "$dummy_keypair" --no-passphrase --force --silent
domichain config set --keypair $dummy_keypair
...
domichain-keygen new -o "$online_system_account_keypair" --no-passphrase --force --silent
domichain-keygen new -o "$online_nonce_account_keypair" --no-passphrase --force --silent

online_system_account_pubkey="$(domichain-keygen pubkey $online_system_account_keypair)"
nonce_account_pubkey="$(domichain-keygen pubkey $online_nonce_account_keypair)"
...
domichain-keygen new -o "$offline_system_account_keypair" --no-passphrase --force --silent
domichain-keygen new -o "$offline_staker_keypair" --no-passphrase --force --silent
domichain-keygen new -o "$offline_withdrawer_keypair" --no-passphrase --force --silent
domichain-keygen new -o "$offline_custodian_keypair" --no-passphrase --force --silent

offline_system_account_pubkey="$(domichain-keygen pubkey $offline_system_account_keypair)"
offline_withdrawer_pubkey="$(domichain-keygen pubkey $offline_withdrawer_keypair)"
offline_staker_pubkey="$(domichain-keygen pubkey $offline_staker_keypair)"
offline_custodian_pubkey="$(domichain-keygen pubkey $offline_custodian_keypair)"

# Airdrop some funds to the offline account.
domichain airdrop 100 $offline_system_account_pubkey
domichain airdrop 2 $online_system_account_pubkey

# Create a nonce account funded by the online account, with the authority given to the offline account
domichain create-nonce-account $online_nonce_account_keypair 1 --nonce-authority $offline_system_account_pubkey --keypair $online_system_account_keypair
nonce="$(domichain nonce $nonce_account_pubkey)"

execution_step OFFLINE SYSTEM ACCOUNT BALANCE BEFORE CREATING STAKE ACCOUNTS
(
  set -x
  domichain balance $offline_system_account_pubkey
)
...
domichain-keygen new -o "$stake_account_keypair" --no-passphrase --force --silent
stake_account_address="$(domichain-keygen pubkey $stake_account_keypair)"

sign_only="$(domichain create-stake-account $stake_account_keypair 50 \
...
domichain create-stake-account $stake_account_keypair 50 \
...
domichain-keygen new -o $split_stake_account_keypair --no-passphrase --force --silent
split_stake_account_address=$(domichain-keygen pubkey $split_stake_account_keypair)

nonce="$(domichain nonce $nonce_account_pubkey)"

sign_only="$(domichain split-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
domichain split-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
...
(
  set -x
  domichain stake-account $split_stake_account_address
)
...
sign_only="$(domichain stake-set-lockup --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
  $split_stake_account_address --custodian $offline_custodian_keypair --lockup-epoch 0 \
  --keypair $offline_system_account_keypair --sign-only --url http://0.0.0.0)"

signers="$(get_signers_string "${sign_only[@]}")"

domichain stake-set-lockup --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
# Withdraw the satomis from the stake account that was split off and return them to the offline system account

nonce="$(domichain nonce $nonce_account_pubkey)"

sign_only="$(domichain withdraw-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
domichain withdraw-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
  $split_stake_account_address $offline_system_account_pubkey 10 \
...
vote_account_pubkey="$(awk '{if(NR==4) print $2}'<<<"$(domichain show-validators)")"
nonce="$(domichain nonce $nonce_account_pubkey)"

# Sign a stake delegation, assuming the authorized staker is held offline
sign_only="$(domichain delegate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
domichain delegate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
...
# Sign a stake delegation, assuming the authorized staker is held offline
sign_only="$(domichain deactivate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
# Send the signed transaction on the cluster
domichain deactivate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
