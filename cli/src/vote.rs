use {
    ...
    domichain_rpc_client_api::config::RpcGetVoteAccountsConfig,
...
pub fn process_create_vote_account(
    ...
    let is_feature_active = (!sign_only)
        .then(domichain_sdk::feature_set::vote_state_add_vote_latency::id)
    ...
    if !sign_only {
        ...
        if let Some(nonce_account) = &nonce_account {
            let nonce_account = domichain_rpc_client_nonce_utils::get_account_with_commitment(
                rpc_client,
                nonce_account,
                config.commitment,
            )?;
