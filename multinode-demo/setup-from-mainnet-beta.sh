...
rm -rf "$DOMICHAIN_CONFIG_DIR"/latest-mainnet-beta-snapshot
mkdir -p "$DOMICHAIN_CONFIG_DIR"/latest-mainnet-beta-snapshot
(
  cd "$DOMICHAIN_CONFIG_DIR"/latest-mainnet-beta-snapshot || exit 1
  set -x
...
snapshot=$(ls "$DOMICHAIN_CONFIG_DIR"/latest-mainnet-beta-snapshot/snapshot-[0-9]*-*.tar.zst)
if [[ -z $snapshot ]]; then
...
$domichain_ledger_tool create-snapshot \
  --ledger "$DOMICHAIN_CONFIG_DIR"/latest-mainnet-beta-snapshot \
