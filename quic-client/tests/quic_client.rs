...
mod tests {
    use {
        ...
        log::*,
        domichain_connection_cache::connection_cache_stats::ConnectionCacheStats,
        domichain_perf::packet::PacketBatch,
        domichain_quic_client::nonblocking::quic_client::{
            QuicClientCertificate, QuicLazyInitializedEndpoint,
        },
        domichain_sdk::{net::DEFAULT_TPU_COALESCE, packet::PACKET_DATA_SIZE, signature::Keypair},
        domichain_streamer::{
    ...
    fn test_quic_client_multiple_writes() {
        use {
            ...
            domichain_quic_client::quic_client::QuicClientConnection,
        };
        domichain_logger::setup();
        let (sender, receiver) = unbounded();
        let staked_nodes = Arc::new(RwLock::new(StakedNodes::default()));
        let (s, exit, keypair, ip) = server_args();
        let (_, t) = domichain_streamer::quic::spawn_server(
            "quic_streamer_test",
            s.try_clone().unwrap(),
            &keypair,
            ip,
    ...
    async fn test_nonblocking_quic_client_multiple_writes() {
        use {
            ...
            domichain_quic_client::nonblocking::quic_client::QuicClientConnection,
        };
        domichain_logger::setup();
        let (sender, receiver) = unbounded();
        let staked_nodes = Arc::new(RwLock::new(StakedNodes::default()));
        let (s, exit, keypair, ip) = server_args();
        let (_, _, t) = domichain_streamer::nonblocking::quic::spawn_server(
            "quic_streamer_test",
            s.try_clone().unwrap(),
            &keypair,
            ip,
    ...
    fn test_quic_bi_direction() {
        ...
        let (request_recv_endpoint, request_recv_thread) = domichain_streamer::quic::spawn_server(
            "quic_streamer_test",
        ...
        let (response_recv_endpoint, response_recv_thread) = domichain_streamer::quic::spawn_server(
            "quic_streamer_test",
