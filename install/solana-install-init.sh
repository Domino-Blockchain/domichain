...
# install domichain-install. It just does platform detection, downloads the installer
# and runs it.

{ # this ensures the entire script is downloaded #

if [ -z "$SOLANA_DOWNLOAD_ROOT" ]; then
    SOLANA_DOWNLOAD_ROOT="https://github.com/domichain-labs/domichain/releases/download/"
fi
...
USAGE:
    domichain-install-init [FLAGS] [OPTIONS] --data_dir <PATH> --pubkey <PUBKEY>
...
OPTIONS:
    ...
    -u, --url <URL>          JSON RPC URL for the domichain cluster
...
main() {
    ...
    *)
    ...
    temp_dir="$(mktemp -d 2>/dev/null || ensure mktemp -d -t domichain-install-init)"
    ...
    download_url="$SOLANA_DOWNLOAD_ROOT/$release/domichain-install-init-$TARGET"
    domichain_install_init="$temp_dir/domichain-install-init"
    ...
    if [ ! -x "$domichain_install_init" ]; then
        ...
        printf '%s\n' "Please copy the file to a location where you can execute binaries and run ./domichain-install-init." 1>&2
        exit 1
    fi

    if [ -z "$1" ]; then
      #shellcheck disable=SC2086
      ignore "$domichain_install_init" $SOLANA_INSTALL_INIT_ARGS
    else
...
err() {
    printf 'domichain-install-init: %s\n' "$1" >&2
    exit 1
}
