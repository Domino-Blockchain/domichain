...
screen -d -m -S validator bash -c "./multinode-demo/validator-x.sh \
  ...
  --allow-private-addr \
  --vote-account ~/vote_account.json \
...
./multinode-demo/delegate-stake.sh --vote-account ~/vote_account.json   --url "http://$NODE_IP_ADDR:8899/"   --label test1   --keypair ~/validator-stake-keypair.json   --force
