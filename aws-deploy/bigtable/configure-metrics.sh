# |source| this file
#
# The DOMI_METRICS_CONFIG environment variable is formatted as a
# comma-delimited list of parameters. All parameters are optional.
#
# Example:
#   export DOMI_METRICS_CONFIG="host=<metrics host>,db=<database name>,u=<username>,p=<password>"
#
...
configureMetrics() {
  [[ -n $DOMI_METRICS_CONFIG ]] || return 0

  declare metricsParams
  IFS=',' read -r -a metricsParams <<< "$DOMI_METRICS_CONFIG"
...
echo "$(dirname "$0")"/metrics-write-datapoint.sh
pwd
# metricsWriteDatapoint="$(dirname "$0")"/metrics-write-datapoint.sh
metricsWriteDatapoint="echo"
