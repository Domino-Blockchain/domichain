...
#shellcheck source=/dev/null
source "$here"/service-env-warehouse*.sh
...
identity_pubkey=$(domichain-keygen pubkey "$identity_keypair")
...
args=(
  ...
  --known-validator 7hoUyAJm3dG7XpFt4yB1f7AHZvaQygyHcb8hD66vP2Ai
  --known-validator 9MyuD1XotiuYWtDHFJnXkPFKxdSL47uMJxYFjBDpFbQo
#  --only-known-rpc
  --full-rpc-api
...
while true; do
  ...
  while true; do
    ...
    if [[ -z "$last_archive_epoch" ]]; then
      if ! domichain --url "$RPC_URL" epoch > ~/ledger-archive/epoch; then
    ...
    for _ in $(seq 1 10); do
      current_epoch=$(domichain --url "$RPC_URL" epoch || true)
        ...
        if ((minutes_since_last_ledger_archive % 60 == 0)); then
          ...
          if [[ -n $latest_snapshot ]]; then
            ...
            if [[ $latest_snapshot_slot = "$last_known_good_snapshot_slot" ]]; then
              ...
              if domichain-ledger-tool --ledger ~/snapshot-check verify; then
    ...
    (
      set -x
      domichain-ledger-tool --ledger "$ledger_dir" bounds | tee ~/ledger-archive/bounds.txt
      domichain-ledger-tool --version | tee ~/ledger-archive/version.txt
    )
