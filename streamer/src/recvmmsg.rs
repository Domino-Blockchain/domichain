...
pub use domichain_perf::packet::NUM_RCVMMSGS;
use {
...
pub fn recv_mmsg(sock: &UdpSocket, packets: &mut [Packet]) -> io::Result</*num packets:*/ usize> {
    ...
    let buffer = std::mem::MaybeUninit::<[iovec; NUM_RCVMMSGS]>::uninit();
    let mut iovs: [iovec; NUM_RCVMMSGS] = unsafe { buffer.assume_init() };
