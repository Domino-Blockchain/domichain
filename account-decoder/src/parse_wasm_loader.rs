...
#[cfg(test)]
mod test {
    use {super::*, bincode::serialize, domichain_sdk::pubkey::Pubkey};

    #[test]
    fn test_parse_wasm_upgradeable_loader_accounts() {
        let wasm_loader_state = UpgradeableLoaderState::Uninitialized;
        let account_data = serialize(&wasm_loader_state).unwrap();
        assert_eq!(
            parse_wasm_upgradeable_loader(&account_data).unwrap(),
            wasmUpgradeableLoaderAccountType::Uninitialized
        );

        let program = vec![7u8; 64]; // Arbitrary program data

        let authority = Pubkey::new_unique();
        let wasm_loader_state = UpgradeableLoaderState::Buffer {
            authority_address: Some(authority),
        };
        let mut account_data = serialize(&wasm_loader_state).unwrap();
        ...
        let wasm_loader_state = UpgradeableLoaderState::Program {
            programdata_address,
        };
        let account_data = serialize(&wasm_loader_state).unwrap();
        assert_eq!(
        ...
        let authority = Pubkey::new_unique();
        let slot = 42;
        let wasm_loader_state = UpgradeableLoaderState::ProgramData {
            slot,
