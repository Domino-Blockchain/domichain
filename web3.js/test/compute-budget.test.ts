...
describe('ComputeBudgetProgram', () => {
  it('requestUnits', () => {
    const params = {
      units: 150000,
      additionalFee: LAMPORTS_PER_DOMI,
    };
  ...
  if (process.env.TEST_LIVE) {
    it('send live request units ix', async () => {
      ...
      const FEE_AMOUNT = LAMPORTS_PER_DOMI;
      const STARTING_AMOUNT = 2 * LAMPORTS_PER_DOMI;
