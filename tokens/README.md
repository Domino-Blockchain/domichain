...
```bash
domichain-tokens distribute-tokens --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
domichain-tokens transaction-log --output-path transactions.csv
```
...
```bash
domichain-tokens distribute-tokens --dry-run --input-csv <RECIPIENTS_CSV>
```
...
```bash
domichain-tokens distribute-tokens --transfer-amount 10 --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
domichain-tokens distribute-stake --stake-account-address <ACCOUNT_ADDRESS> \
...
```bash
domichain-tokens spl-token-balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
...
```bash
domichain-tokens distribute-spl-tokens --from <ADDRESS> --owner <KEYPAIR> \
...
the `--dry-run` parameter, or `domichain-tokens balances`.
...
```bash
domichain-tokens balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
