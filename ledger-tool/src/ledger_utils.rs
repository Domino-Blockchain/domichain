...
pub fn load_and_process_ledger(
    ...
    let account_paths = if let Some(account_paths) = arg_matches.value_of("account_paths") {
        // If this blockstore access is Primary, no other process (domichain-validator) can hold
        // Primary access. So, allow a custom accounts path without worry of wiping the accounts
        // of domichain-validator.
        if !blockstore.is_primary_access() {
            ...
            // the Primary access instead of holding it to give priority to domichain-validator over
            // domichain-ledger-tool should domichain-validator start before we've finished.
            info!(
