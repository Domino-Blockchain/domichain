...
async fn upload(
    ...
    while starting_slot <= ending_slot {
        ...
        let last_slot_checked = domichain_ledger::bigtable_upload::upload_confirmed_blocks(
            blockstore.clone(),
            bigtable.clone(),
            starting_slot,
            current_ending_slot,
                ...
                while let Ok(slot) = r.try_recv() {
                    ...
                    if args.dry_run {
                        ...
                        match source_bigtable_clone.get_confirmed_block(slot).await {
                            ...
                            Err(domichain_storage_bigtable::Error::BlockNotFound(slot)) => {
                                debug!("block not found, slot: {}", slot);
                                block_not_found_slots_clone.lock().unwrap().push(slot);
                                continue;
                            }
...
) -> domichain_storage_bigtable::Result<domichain_storage_bigtable::LedgerStorage> {
    if let Some(endpoint) = args.emulated_source {
        domichain_storage_bigtable::LedgerStorage::new_for_emulator(
            &args.instance_name,
            &args.app_profile_id,
            &endpoint,
            args.timeout,
        ...
        domichain_storage_bigtable::LedgerStorage::new_with_config(
            domichain_storage_bigtable::LedgerStorageConfig {
