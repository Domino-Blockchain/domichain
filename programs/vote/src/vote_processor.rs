...
use {
    ...
    log::*,
    domichain_program::vote::{instruction::VoteInstruction, program::id, state::VoteAuthorize},
...
mod tests {
    ...
    fn create_test_account_with_authorized() -> (Pubkey, Pubkey, Pubkey, AccountSharedData) {
        ...
        let authorized_withdrawer = domichain_sdk::pubkey::new_rand();
    ...
    fn test_authorize_voter() {
        let (vote_pubkey, vote_account) = create_test_account();
        let authorized_voter_pubkey = domichain_sdk::pubkey::new_rand();
        let clock = Clock {
            epoch: 1,
