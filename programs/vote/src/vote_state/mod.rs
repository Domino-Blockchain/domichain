...
pub use domichain_program::vote::state::{vote_state_versions::*, *};
use {
    log::*,
    serde_derive::{Deserialize, Serialize},
    domichain_metrics::datapoint_debug,
    domichain_program::vote::{error::VoteError, program::id, state::serde_compact_vote_state_update},
    domichain_sdk::{
...
mod tests {
    ...
    fn test_vote_state_upgrade_from_1_14_11() {
        ...
        assert_eq!(
            vote_state
                .set_new_authorized_voter(&domichain_sdk::pubkey::new_rand(), 0, 1, |_pubkey| Ok(()),),
            Ok(())
        );
        vote_state.increment_credits(1, 200);
        assert_eq!(
            vote_state
                .set_new_authorized_voter(&domichain_sdk::pubkey::new_rand(), 1, 2, |_pubkey| Ok(()),),
            Ok(())
        );
        vote_state.increment_credits(2, 300);
        assert_eq!(
            vote_state
                .set_new_authorized_voter(&domichain_sdk::pubkey::new_rand(), 2, 3, |_pubkey| Ok(()),),
            Ok(())
        );
