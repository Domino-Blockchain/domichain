...
use {
    ...
    domichain_sdk::{
        ...
        blake3, wasm_loader, wasm_loader_deprecated, wasm_loader_upgradeable,
...
fn translate_and_check_program_address_inputs<'a>(
    ...
    let seeds_serialized = translate_slice::<u8>(
        memory_mapping,
        seeds_addr,
        seeds_len,
        check_aligned,
    ...
    let seeds = bincode::deserialize(seeds_serialized).unwrap();
...
declare_syscall!(
    ...
    fn inner_call(
        ...
        use domichain_sdk::alt_bn128::prelude::{ALT_BN128_ADD, ALT_BN128_MUL, ALT_BN128_PAIRING};
        let budget = invoke_context.get_compute_budget();
...
mod tests {
    ...
    use {
        ...
        core::slice,
        domichain_program_runtime::{invoke_context::InvokeContext, with_mock_invoke_context},
        solana_rbpf::{
            error::EbpfError,
    ...
    fn test_syscall_abort() {
        prepare_mockup!(invoke_context, program_id, wasm_loader::id());
        let config = Config::default();
    ...
    fn test_syscall_sol_alloc_free() {
        // large alloc
        {
            ...
            SyscallAllocFree::call(
                invoke_context,
                domichain_sdk::entrypoint::HEAP_LENGTH as u64,
                0,
                0,
                0,
                0,
    ...
    fn test_syscall_sha256() {
        let config = Config::default();
        prepare_mockup!(invoke_context, program_id, wasm_loader_deprecated::id());
    ...
    fn test_syscall_edwards_curve_point_validation() {
        use domichain_zk_token_sdk::curve25519::curve_syscall_traits::CURVE25519_EDWARDS;
    ...
    fn test_syscall_ristretto_curve_point_validation() {
        use domichain_zk_token_sdk::curve25519::curve_syscall_traits::CURVE25519_RISTRETTO;
    ...
    fn test_syscall_edwards_curve_group_ops() {
        use domichain_zk_token_sdk::curve25519::curve_syscall_traits::{
    ...
    fn test_syscall_sol_get_processed_sibling_instruction() {
        let transaction_accounts = (0..9)
            .map(|_| {
                (
                    Pubkey::new_unique(),
                    AccountSharedData::new(0, 0, &wasm_loader::id()),
                )
            })
            .collect::<Vec<_>>();
    ...
    fn test_create_program_address() {
        ...
        prepare_mockup!(invoke_context, program_id, wasm_loader::id());
        let address = wasm_loader_upgradeable::id();
