...
use {
    ...
    solana_rbpf::{
        ...
        vm::{BuiltinProgram, ContextObject, EbpfVm, ProgramResult, BuiltinFunction},
    },
    domichain_sdk::{
...
pub fn load_program_from_account(
    ...
    if !check_loader_id(program.get_owner()) {
        ic_logger_msg!(
            log_collector,
            "Executable account not owned by the WASM loader"
        );
        return Err(InstructionError::IncorrectProgramId);
    }

    let (programdata_offset, deployment_slot) =
        if wasm_loader_upgradeable::check_id(program.get_owner()) {
...
fn process_instruction_inner(
        ...
        .is_active(&remove_bpf_loader_incorrect_program_id::id())
    {
                ...
                .unwrap_or(false)
        {
            ...
            if first_account.is_executable() {
                ic_logger_msg!(log_collector, "WASM loader is executable");
    ...
    if native_loader::check_id(program_account.get_owner()) {
        ...
        let program_id = instruction_context.get_last_program_key(transaction_context)?;
        return if wasm_loader_upgradeable::check_id(program_id) {
...
fn map_wasmi_to_bpf_syscalls<'a, 'b>(
    ...
    // let invoke_context: &'a mut InvokeContext<'b> = invoke_context.into_inner();
    // <InvokeContext as ContextObject>::consume(&mut invoke_context, 0);

    let invoke_context_ref = Rc::as_ptr(&invoke_context);
...
fn execute<'a, 'b: 'a>(
    ...
    let execution_result = {
        ...
        {
            let feature_set = invoke_context.borrow().feature_set.clone();
        ...
        //     "env",
        //     "sol_log_",
        //     Func::wrap(&mut store, |mut caller: Caller<'_, HostState>, message: i32, len: u64| {
        //         let mem = match caller.get_export("memory") {
        //             Some(Extern::Memory(mem)) => mem,
        //             _ => panic!("failed to find host memory"),
        //         };
        ...
        //         let message_end = message as usize + len as usize;

        //         println!("[{}:{}] {}", file!(), line!(), std::str::from_utf8(&data[message_start..message_end]).unwrap());
    ...
    let invoke_context = RefCell::into_inner(Rc::into_inner(invoke_context).unwrap());
...
pub mod test_utils {
    use {
        super::*, domichain_program_runtime::loaded_programs::DELAY_VISIBILITY_SLOT_OFFSET,
        domichain_sdk::account::ReadableAccount,
    };
...
mod tests {
    ...
    fn test_wasm_loader_check_load_dw() {
        let prog = &[
    ...
    #[test]
    fn test_wasm_loader_write() {
        let loader_id = wasm_loader::id();
    ...
    #[test]
    fn test_wasm_loader_invoke_main() {
        let loader_id = wasm_loader::id();
        let program_id = Pubkey::new_unique();
        let mut program_account =
            load_program_account_from_elf(&loader_id, "test_elfs/out/noop_aligned.so");
    ...
    #[test]
    fn test_wasm_loader_serialize_unaligned() {
        let loader_id = wasm_loader_deprecated::id();
        let program_id = Pubkey::new_unique();
        let program_account =
    ...
    #[test]
    fn test_wasm_loader_serialize_aligned() {
        let loader_id = wasm_loader::id();
        let program_id = Pubkey::new_unique();
        let program_account =
            load_program_account_from_elf(&loader_id, "test_elfs/out/noop_aligned.so");
    ...
    #[test]
    fn test_wasm_loader_upgradeable_initialize_buffer() {
    ...
    #[test]
    fn test_wasm_loader_upgradeable_write() {
        let loader_id = wasm_loader_upgradeable::id();
        let buffer_address = Pubkey::new_unique();
    ...
    #[test]
    fn test_wasm_loader_upgradeable_upgrade() {
    ...
    #[test]
    fn test_wasm_loader_upgradeable_set_upgrade_authority() {
    ...
    #[test]
    fn test_wasm_loader_upgradeable_set_upgrade_authority_checked() {
        let instruction =
    ...
    #[test]
    fn test_wasm_loader_upgradeable_set_buffer_authority() {
        let instruction = bincode::serialize(&UpgradeableLoaderInstruction::SetAuthority).unwrap();
        let loader_id = wasm_loader_upgradeable::id();
    ...
    #[test]
    fn test_wasm_loader_upgradeable_set_buffer_authority_checked() {
        let instruction =
            bincode::serialize(&UpgradeableLoaderInstruction::SetAuthorityChecked).unwrap();
        let loader_id = wasm_loader_upgradeable::id();
        let invalid_authority_address = Pubkey::new_unique();
    ...
    #[test]
    fn test_wasm_loader_upgradeable_close() {
