...
use {
    domichain_wasm_loader_program::serialization::serialize_parameters,
    domichain_sdk::{
...
fn bench_serialize_unaligned(bencher: &mut Bencher) {
    let transaction_context = create_inputs(wasm_loader_deprecated::id(), 7);
...
fn bench_serialize_aligned(bencher: &mut Bencher) {
    let transaction_context = create_inputs(wasm_loader::id(), 7);
    let instruction_context = transaction_context
        .get_current_instruction_context()
        .unwrap();
...
fn bench_serialize_unaligned_max_accounts(bencher: &mut Bencher) {
    let transaction_context = create_inputs(wasm_loader_deprecated::id(), 255);
    let instruction_context = transaction_context
        .get_current_instruction_context()
        .unwrap();
    bencher.iter(|| {
...
fn bench_serialize_aligned_max_accounts(bencher: &mut Bencher) {
    let transaction_context = create_inputs(wasm_loader::id(), 255);
    let instruction_context = transaction_context
        .get_current_instruction_context()
        .unwrap();
