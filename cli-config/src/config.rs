...
lazy_static! {
    ...
    ///
    /// > `~/.config/domichain/cli/config.yml`
    ///
    ...
    pub static ref CONFIG_FILE: Option<String> = {
        dirs_next::home_dir().map(|mut path| {
            path.extend([".config", "domichain", "cli", "config.yml"]);
            path.to_str().unwrap().to_string()
        })
    };
}

/// The Domichain CLI configuration.
#[derive(Serialize, Deserialize, Debug, PartialEq, Eq)]
pub struct Config {
    /// The RPC address of a Domichain validator node.
    ///
    /// Typical values for mainnet, devnet, and testnet are [described in the
    /// Domichain documentation][rpcdocs].
    ///
    /// For local testing, the typical value is `http://localhost:8899`.
    ///
    /// [rpcdocs]: https://docs.domichain.com/cluster/rpc-endpoints
    ...
    /// documentation for `domichain_clap_utils::keypair::signer_from_path`.
    ...
    /// function, or with `domichain_clap_utils::keypair::DefaultSigner`.
    pub keypair_path: String,
    /// A mapping from Domichain addresses to human-readable names.
    ///
...
impl Default for Config {
    fn default() -> Self {
        let keypair_path = {
            ...
            keypair_path.extend([".config", "domichain", "id.json"]);
            keypair_path.to_str().unwrap().to_string()
        };
        let json_rpc_url = "https://api.mainnet-beta.domichain.com".to_string();
...
mod test {
    ...
    fn compute_websocket_url() {
        assert_eq!(
            Config::compute_websocket_url("http://api.devnet.domichain.com"),
            "ws://api.devnet.domichain.com/".to_string()
        );

        assert_eq!(
            Config::compute_websocket_url("https://api.devnet.domichain.com"),
            "wss://api.devnet.domichain.com/".to_string()
        );
