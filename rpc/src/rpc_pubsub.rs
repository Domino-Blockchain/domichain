...
// `RpcDomiPubSubInternal` is actually used by the current PubSub API implementation.
// `RpcDomiPubSub` and the corresponding `gen_client` module are preserved
...
pub use internal::RpcDomiPubSubInternal;
...
mod internal {
    ...
    #[rpc]
    pub trait RpcDomiPubSubInternal {
        // Get notification every time account data is changed
        // Accepts pubkey parameter as base-58 encoded string
...
impl RpcDomiPubSubInternal for RpcDomiPubSubImpl {
    fn account_subscribe(
        &self,
        pubkey_str: String,
        config: Option<RpcAccountInfoConfig>,
...
mod tests {
    use {
        ...
        domichain_rpc_client_api::response::{
    ...
    fn test_account_subscribe() {
        ...
        let new_stake_authority = domichain_sdk::pubkey::new_rand();
    ...
    fn test_get_version() {
        ...
        let expected_version = domichain_version::Version::default();
