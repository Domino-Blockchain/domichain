//! The `rpc` module implements the Domichain RPC interface.
...
impl JsonRpcRequestProcessor {
    ...
    fn bank(&self, commitment: Option<CommitmentConfig>) -> Arc<Bank> {
        ...
        r_bank_forks.get(slot).unwrap_or_else(|| {
            ...
            // a slot.
            //
            // For more information, see https://Domino-Blockchain/domichain/issues/11078
            warn!(
    ...
    pub fn new_from_bank(
        ...
        let cluster_info = Arc::new({
            ...
            let contact_info = ContactInfo::new_localhost(
                &keypair.pubkey(),
                domichain_sdk::timing::timestamp(), // wallclock
            );
            ...
            .map(|address| {
                if let Some(reward) = reward_hash.get(address) {
                    return Some(RpcInflationReward {
                        ...
                        amount: reward.satomis.unsigned_abs(),
    ...
    fn get_recent_blockhash(
        ...
        let satomis_per_signature = bank
    ...
    fn get_fee_calculator_for_blockhash(
        ...
        let bank = self.bank(commitment);
        let satomis_per_signature = bank.get_satomis_per_signature_for_blockhash(blockhash);
        Ok(new_response(
            &bank,
            satomis_per_signature.map(|satomis_per_signature| RpcFeeCalculator {
                fee_calculator: FeeCalculator::new(satomis_per_signature),
            }),
        ))
    }
    ...
    fn get_supply(
        ...
        Ok(new_response(
            ...
            RpcSupply {
                total: total_supply,
                circulating: total_supply - non_circulating_supply.satomis,
                non_circulating: non_circulating_supply.satomis,
    ...
    pub fn get_confirmed_signatures_for_address(
        ...
        if self.config.enable_rpc_transaction_history {
            ...
            // https://Domino-Blockchain/domichain/pull/10928
            let end_slot = min(
                end_slot,
    ...
    pub fn get_stake_activation(
        ...
        let delegation = match delegation {
            None => {
                return Ok(RpcStakeActivation {
                    ...
                    active: 0,
                    inactive: stake_account.satomis().saturating_sub(rent_exempt_reserve),
                })
            }
        ...
        let inactive_stake = match stake_activation_state {
            ...
            StakeActivationState::Inactive => {
                stake_account.satomis().saturating_sub(rent_exempt_reserve)
            }
        };
            ...
            Ok(bank
                .get_filtered_indexed_accounts(
                    ...
                    |account| {
                        ...
                        // zero-satomi AccountSharedData::Default() after being wiped and reinitialized in later
    ...
    fn get_filtered_spl_token_accounts_by_owner(
        ...
        // in storage as a zero-satomi AccountSharedData::Default() after being wiped and reinitialized in
    ...
    fn get_filtered_spl_token_accounts_by_mint(
        ...
        // as be zero-satomi AccountSharedData::Default() after being wiped and reinitialized in later
...
pub mod rpc_accounts {
    ...
    pub trait AccountsData {
        ...
        // See https://Domino-Blockchain/domichain-program-library/releases/tag/token-v2.0.0 for
        // program details
...
pub mod rpc_full {
    use {
        super::*,
        domichain_sdk::message::{SanitizedVersionedMessage, VersionedMessage},
    };
    #[rpc]
    pub trait Full {
        type Metadata;
    ...
    impl Full for FullImpl {
        ...
        fn request_airdrop(
            ...
            trace!(
                "request_airdrop id={} satomis={} config: {:?}",
                pubkey_str,
                satomis,
                &config
            );
            ...
            let transaction =
                request_airdrop_transaction(&faucet_addr, &pubkey, satomis, blockhash).map_err(
                    |err| {
...
pub fn populate_blockstore_for_tests(
    ...
    let shreds = domichain_ledger::blockstore::entries_to_test_shreds(
        &entries,
        slot,
        parent_slot,
        true,
...
pub mod tests {
    ...
    const TEST_MINT_SATOMIS: u64 = 1_000_000_000;
    ...
    fn test_get_largest_accounts() {
        ...
        assert!(largest_accounts_result.value.contains(&RpcAccountBalance {
            ...
            satomis: mint_balance_result.value,
        }));
        ...
        assert!(largest_accounts_result.value.contains(&RpcAccountBalance {
            ...
            satomis: non_circulating_balance_result.value,
        }));
        ...
        let expected = json!({
            ...
            "result": {
                ...
                "value":{
                    ...
                    "feeCalculator": {
                        "satomisPerSignature": TEST_SIGNATURE_FEE,
                    }
                },
            },
            "id": 1
    ...
    fn test_rpc_get_fee_calculator_for_blockhash() {
        ...
        let satomis_per_signature = bank.get_satomis_per_signature();
            ...
            "result": {
                ...
                "value":{
                    "feeRateGovernor": {
                        ...
                        "maxSatomisPerSignature": TEST_SIGNATURE_FEE,
                        "minSatomisPerSignature": TEST_SIGNATURE_FEE,
                        "targetSatomisPerSignature": TEST_SIGNATURE_FEE,
    ...
    fn new_bank_forks_with_config(
        ...
        let GenesisConfigInfo {
            ...
            voting_keypair,
            ..
        } = create_genesis_config(TEST_MINT_SATOMIS);
    ...
    fn test_decode_and_deserialize_too_large_payloads_fail() {
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx58, TransactionBinaryEncoding::Base58)
                .unwrap_err(),
            Error::invalid_params(format!(
                "base58 encoded domichain_sdk::transaction::Transaction too large: {tx58_len} bytes (max: encoded/raw {MAX_BASE58_SIZE}/{PACKET_DATA_SIZE})",
            )
        ));
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx64, TransactionBinaryEncoding::Base64)
                .unwrap_err(),
            Error::invalid_params(format!(
                "base64 encoded domichain_sdk::transaction::Transaction too large: {tx64_len} bytes (max: encoded/raw {MAX_BASE64_SIZE}/{PACKET_DATA_SIZE})",
            )
        ));
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx58, TransactionBinaryEncoding::Base58)
                .unwrap_err(),
            Error::invalid_params(format!(
                "decoded domichain_sdk::transaction::Transaction too large: {too_big} bytes (max: {PACKET_DATA_SIZE} bytes)"
            ))
        );
        ...
        assert_eq!(
            decode_and_deserialize::<Transaction>(tx64.clone(), TransactionBinaryEncoding::Base64)
                .unwrap_err(),
            Error::invalid_params(
                "failed to deserialize domichain_sdk::transaction::Transaction: invalid value: \
