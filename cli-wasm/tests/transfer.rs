...
fn test_transfer() {
    ...
    request_and_confirm_airdrop(&rpc_client, &config, &sender_pubkey, domi_to_satomis(5.0))
        .unwrap();
    check_balance!(domi_to_satomis(5.0), &rpc_client, &sender_pubkey);
    ...
    config.command = CliCommand::Transfer {
        amount: SpendAmount::Some(domi_to_satomis(1.0)),
        to: recipient_pubkey,
        from: 0,
        sign_only: false,
    ...
    check_balance!(
        domi_to_satomis(4.0) - fee_one_sig,
        &rpc_client,
        &sender_pubkey
    );
    check_balance!(domi_to_satomis(1.0), &rpc_client, &recipient_pubkey);

    // Plain ole transfer, failure due to InsufficientFundsForSpendAndFee
    config.command = CliCommand::Transfer {
        amount: SpendAmount::Some(domi_to_satomis(4.0)),
        to: recipient_pubkey,
        from: 0,
        sign_only: false,
        dump_transaction_message: false,
    ...
    request_and_confirm_airdrop(&rpc_client, &offline, &offline_pubkey, domi_to_satomis(1.0))
        .unwrap();
    check_balance!(domi_to_satomis(1.0), &rpc_client, &offline_pubkey);

    // Offline transfer
    let blockhash = rpc_client.get_latest_blockhash().unwrap();
    offline.command = CliCommand::Transfer {
        amount: SpendAmount::Some(domi_to_satomis(0.5)),
        to: recipient_pubkey,
        from: 0,
        sign_only: true,
        dump_transaction_message: false,
    ...
    process_command(&config).unwrap();
    check_balance!(
        domi_to_satomis(0.5) - fee_one_sig,
        &rpc_client,
        &offline_pubkey
    );
    check_balance!(domi_to_satomis(1.5), &rpc_client, &recipient_pubkey);
    ...
    process_command(&config).unwrap();
    check_balance!(
        domi_to_satomis(4.0) - fee_one_sig - fee_two_sig - minimum_nonce_balance,
        &rpc_client,
        &sender_pubkey,
    );
    ...
    process_command(&config).unwrap();
    check_balance!(
        domi_to_satomis(3.0) - 2 * fee_one_sig - fee_two_sig - minimum_nonce_balance,
        &rpc_client,
        &sender_pubkey,
    );
    check_balance!(domi_to_satomis(2.5), &rpc_client, &recipient_pubkey);
    ...
    process_command(&config).unwrap();
    check_balance!(
        domi_to_satomis(3.0) - 3 * fee_one_sig - fee_two_sig - minimum_nonce_balance,
        &rpc_client,
        &sender_pubkey,
    );
    ...
    offline.signers = vec![&default_offline_signer];
    offline.command = CliCommand::Transfer {
        amount: SpendAmount::Some(domi_to_satomis(0.4)),
        to: recipient_pubkey,
        from: 0,
        sign_only: true,
        dump_transaction_message: false,
    ...
    process_command(&config).unwrap();
    check_balance!(
        domi_to_satomis(0.1) - 2 * fee_one_sig,
        &rpc_client,
        &offline_pubkey
    );
    check_balance!(domi_to_satomis(2.9), &rpc_client, &recipient_pubkey);
}
...
fn test_transfer_multisession_signing() {
    ...
    fee_payer_config.command = CliCommand::Transfer {
        amount: SpendAmount::Some(domi_to_satomis(42.0)),
        to: to_pubkey,
        from: 1,
        sign_only: true,
        dump_transaction_message: false,
    ...
    check_balance!(
        domi_to_satomis(1.0) + fee,
        &rpc_client,
        &offline_fee_payer_signer.pubkey(),
    );
    check_balance!(domi_to_satomis(42.0), &rpc_client, &to_pubkey);
}
...
fn test_transfer_with_seed() {
    ...
    request_and_confirm_airdrop(&rpc_client, &config, &sender_pubkey, domi_to_satomis(1.0))
        .unwrap();
    request_and_confirm_airdrop(&rpc_client, &config, &derived_address, domi_to_satomis(5.0))
        .unwrap();
    check_balance!(domi_to_satomis(1.0), &rpc_client, &sender_pubkey);
    check_balance!(domi_to_satomis(5.0), &rpc_client, &derived_address);
    check_balance!(0, &rpc_client, &recipient_pubkey);
    ...
    // Transfer with seed
    config.command = CliCommand::Transfer {
        amount: SpendAmount::Some(domi_to_satomis(5.0)),
        to: recipient_pubkey,
        from: 0,
        sign_only: false,
        dump_transaction_message: false,
    ...
    process_command(&config).unwrap();
    check_balance!(domi_to_satomis(1.0) - fee, &rpc_client, &sender_pubkey);
    check_balance!(domi_to_satomis(5.0), &rpc_client, &recipient_pubkey);
