...
impl ProgramSubCommands for App<'_, '_> {
    fn program_subcommands(self) -> Self {
        ...
        .subcommand(
            SubCommand::with_name("deploy")
                .about("Deploy has been removed. Use `domichain program deploy` instead to deploy upgradeable programs")
...
fn read_and_verify_wasm(program_location: &str) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    ...
    let _module = wasmi::Module::new(&engine, &mut &program_data[data_offset..])
        .map_err(|err| format!("WASM error: {err}"))?;
...
fn report_ephemeral_mnemonic(words: usize, mnemonic: bip39::Mnemonic) {
    ...
    eprintln!("`domichain-keygen recover` and the following {words}-word seed phrase:");
    eprintln!("{divider}\n{phrase}\n{divider}");
