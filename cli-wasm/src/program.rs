...
use domichain_sdk::{bpf_loader, bpf_loader_deprecated, bpf_loader_upgradeable};
...
fn process_program_deploy(
    ...
    let do_deploy = if let Some(account) = rpc_client
        ...
        .value
    {
        if account.owner != wasm_loader_upgradeable::id() && account.owner != bpf_loader_upgradeable::id() {
            return Err(format!(
    ...
    let (program_data, program_len, loader_id) = if let Some(program_location) = program_location {
        let program_location = Path::new(program_location);
        let (program_data, loader_id) = read_and_verify(program_location)?;
        let program_len = program_data.len();
        (program_data, program_len, loader_id)
        ...
        if let Some(account) = rpc_client
            ...
            .value
        {
            let is_wasm = wasm_loader_upgradeable::check_id(&account.owner);
            let is_bpf = bpf_loader_upgradeable::check_id(&account.owner);
            if !is_wasm && !is_bpf {
                return Err(format!(
                    "Buffer account {buffer_pubkey} is not owned by the WASM or BPF Upgradeable Loader",
                )
                .into());
            }
...
pub fn process_deploy(
    ...
    let loader_id = match (loader_id, use_deprecated_loader) {
        (loader_id, true) if loader_id == wasm_loader_upgradeable::id() => wasm_loader_deprecated::id(),
        (loader_id, false) if loader_id == wasm_loader_upgradeable::id() => wasm_loader::id(),
        (loader_id, true) if loader_id == bpf_loader_upgradeable::id() => bpf_loader_deprecated::id(),
        (loader_id, false) if loader_id == bpf_loader_upgradeable::id() => bpf_loader::id(),
        (_, _) => todo!(),
    };
...
fn do_process_program_write_and_deploy(
        ...
        if let Some(buffer_authority_signer) = buffer_authority_signer {
                ...
                .value
            {
                complete_partial_program_init(
                    ...
                    buffer_pubkey,
                    &account,
                    if loader_id == &wasm_loader_upgradeable::id() || loader_id == &bpf_loader_upgradeable::id() {
    ...
    let get_pubkey = |s: Option<&dyn Signer>| s.map(|s| s.pubkey());
...
fn do_process_program_upgrade(
    ...
    let (initial_message, write_messages, balance_needed) =
        if let Some(buffer_signer) = buffer_signer {
                ...
                .value
            {
                ...
                let buffer = if loader_id == &wasm_loader_upgradeable::id() {
                    wasm_loader_upgradeable::create_buffer(
                        &config.signers[0].pubkey(),
                        buffer_pubkey,
            ...
            let payer_pubkey = config.signers[0].pubkey();
            let create_msg = |offset: u32, bytes: Vec<u8>| {
                let write_fn = if loader_id == &wasm_loader_upgradeable::id() {
                    wasm_loader_upgradeable::write
                } else if loader_id == &bpf_loader_upgradeable::id() {
                    bpf_loader_upgradeable::write
                } else {
    ...
    let upgrade_fn = if loader_id == &wasm_loader_upgradeable::id() {
...
fn read_and_verify<P: AsRef<Path>>(program_location: P) -> Result<(Vec<u8>, Pubkey), Box<dyn std::error::Error>> {
    match program_location.as_ref().extension().map(|s| s.to_str()).flatten() {
        Some("wasm") => Ok((read_and_verify_wasm(program_location)?, wasm_loader_upgradeable::id())),
        Some("so") => Ok((read_and_verify_elf(program_location)?, bpf_loader_upgradeable::id())),
        Some(_) | None => panic!("Wrong file extension. Expected .wasm or .so files"),
    }
}

fn read_and_verify_elf<P: AsRef<Path>>(program_location: P) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
...
fn read_and_verify_wasm<P: AsRef<Path>>(program_location: P) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    let mut file = File::open(program_location)
        .map_err(|err| format!("Unable to open program file: {}", err))?;
    let mut program_data = Vec::new();
    file.read_to_end(&mut program_data)
