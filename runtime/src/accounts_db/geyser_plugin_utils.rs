...
pub mod tests {
    ...
    fn test_notify_account_restore_from_snapshot_once_per_slot() {
        ...
        let mut account1_satomis: u64 = 1;
        let account1 =
            AccountSharedData::new(account1_satomis, 1, AccountSharedData::default().owner());
        let slot0 = 0;
        accounts.store_uncached(slot0, &[(&key1, &account1)]);

        account1_satomis = 2;
        let account1 = AccountSharedData::new(account1_satomis, 1, account1.owner());
        accounts.store_uncached(slot0, &[(&key1, &account1)]);
        let notifier = GeyserTestPlugin::default();

        let key2 = domichain_sdk::pubkey::new_rand();
        let account2_satomis: u64 = 100;
        let account2 =
            AccountSharedData::new(account2_satomis, 1, AccountSharedData::default().owner());
    ...
    fn test_notify_account_restore_from_snapshot_once_across_slots() {
        ...
        let account3_satomis: u64 = 300;
        let account3 =
            AccountSharedData::new(account3_satomis, 1, AccountSharedData::default().owner());
    ...
    fn test_notify_account_at_accounts_update() {
        ...
        // Account with key3 is updated in slot1, should get notified once
        let key1 = domichain_sdk::pubkey::new_rand();
        let account1_satomis1: u64 = 1;
        let account1 =
            AccountSharedData::new(account1_satomis1, 1, AccountSharedData::default().owner());
        let slot0 = 0;
        ...
        let account1_satomis2 = 2;
        let slot1 = 1;
        let account1 = AccountSharedData::new(account1_satomis2, 1, account1.owner());
