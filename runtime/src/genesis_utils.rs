...
const VALIDATOR_SATOMIS: u64 = 42;

// fun fact: rustc is very close to make this const fn.
pub fn bootstrap_validator_stake_satomis() -> u64 {
    Rent::default().minimum_balance(StakeState::size_of())
}

// Number of satomis automatically used for genesis accounts
pub const fn genesis_sysvar_and_builtin_program_satomis() -> u64 {
...
pub fn create_genesis_config(mint_satomis: u64) -> GenesisConfigInfo {
    // Note that zero satomis for validator stake will result in stake account
...
pub fn create_genesis_config_with_vote_accounts_and_cluster_type(
    ...
    for (validator_voting_keypairs, stake) in voting_keypairs[1..].iter().zip(&stakes[1..]) {
        ...
        // Create accounts
        let node_account = Account::new(VALIDATOR_SATOMIS, 0, &system_program::id());
...
pub fn create_genesis_config_with_leader_ex(
    ...
    initial_accounts.push((
        *mint_pubkey,
        AccountSharedData::new(mint_satomis, 0, &system_program::id()),
    ));
    initial_accounts.push((
        *validator_pubkey,
        AccountSharedData::new(validator_satomis, 0, &system_program::id()),
    ));
