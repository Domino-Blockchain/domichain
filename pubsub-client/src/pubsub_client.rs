...
//! `--rpc-pubsub-enable-vote-subscription` to `domichain-validator`. When these
...
//! }
//! #
//! # get_account_updates(domichain_sdk::pubkey::new_rand());
//! # Ok::<(), anyhow::Error>(())
//! ```
...
impl PubsubClient {
    ...
    /// `--rpc-pubsub-enable-block-subscription` to `domichain-validator`.
    ///
    /// # RPC Reference
    ///
