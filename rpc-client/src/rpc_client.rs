...
/// A client of a remote Domichain node.
///
/// `RpcClient` communicates with a Domichain node over [JSON-RPC], with the
/// [Domichain JSON-RPC protocol][jsonprot]. It is the primary Rust interface for
...
impl RpcClient {
    ...
    /// behaviors determined by internal Domichain test cases. New users should
    ...
    /// Returns the stake minimum delegation, in satomis, based on the commitment level.
    ///
    /// # RPC Reference
    ///
    /// This method corresponds directly to the [`getStakeMinimumDelegation`] RPC method.
    ...
    pub fn request_airdrop(&self, pubkey: &Pubkey, satomis: u64) -> ClientResult<Signature> {
        self.invoke((self.rpc_client.as_ref()).request_airdrop(pubkey, satomis))
    }
    ...
    pub fn request_airdrop_with_config(
        ...
        self.invoke(
            (self.rpc_client.as_ref()).request_airdrop_with_config(pubkey, satomis, config),
        )
    }
