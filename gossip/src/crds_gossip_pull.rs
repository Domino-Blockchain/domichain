...
impl domichain_sdk::sanitize::Sanitize for CrdsFilter {
    fn sanitize(&self) -> std::result::Result<(), domichain_sdk::sanitize::SanitizeError> {
    ...
    pub(crate) fn new_pull_request(
        ...
        let (weights, nodes): (Vec<u64>, Vec<ContactInfo>) =
            crds_gossip::dedup_gossip_addresses(nodes, stakes)
                .into_values()
                .map(|(stake, node)| {
                    let stake = stake.min(stake_cap) / SATOMIS_PER_DOMI;
...
pub struct CrdsTimeouts<'a> {
    pubkey: Pubkey,
    stakes: &'a HashMap<Pubkey, /*satomis:*/ u64>,
    default_timeout: u64,
...
pub(crate) mod tests {
    ...
    fn test_hash_as_u64_random() {
        ...
        for _ in 0..100 {
            let hash = domichain_sdk::hash::new_rand(&mut rng);
    ...
    fn test_crds_filter_set_add() {
        ...
        let hash_values: Vec<_> = repeat_with(|| domichain_sdk::hash::new_rand(&mut rng))
            .take(1024)
            .collect();
    ...
    fn test_new_pull_request() {
        ...
        let new = ContactInfo::new_localhost(&domichain_sdk::pubkey::new_rand(), now);
        ping_cache
            .lock()
            .unwrap()
        ...
        let offline = ContactInfo::new_localhost(&domichain_sdk::pubkey::new_rand(), now);
    ...
    fn test_new_mark_creation_time() {
        ...
        let old = ContactInfo::new_localhost(&domichain_sdk::pubkey::new_rand(), 0);
        ...
        crds.insert(old.clone(), now, GossipRoute::LocalMessage)
            .unwrap();
        let new = ContactInfo::new_localhost(&domichain_sdk::pubkey::new_rand(), 0);
    ...
    fn test_process_pull_request_response() {
        ...
        let new = ContactInfo::new_localhost(&domichain_sdk::pubkey::new_rand(), 1);
        ping_cache.mock_pong(*new.pubkey(), new.gossip().unwrap(), Instant::now());
        let new = CrdsValue::new_unsigned(CrdsData::LegacyContactInfo(new));
        node_crds.insert(new, 0, GossipRoute::LocalMessage).unwrap();

        let mut dest_crds = Crds::default();
        let new_id = domichain_sdk::pubkey::new_rand();
    ...
    fn test_gossip_purge() {
        ...
        let mut node_crds = Crds::default();
        let entry = CrdsValue::new_unsigned(CrdsData::LegacyContactInfo(
            ContactInfo::new_localhost(&domichain_sdk::pubkey::new_rand(), 0),
        ));
    ...
    fn test_process_pull_response() {
        ...
        let peer_pubkey = domichain_sdk::pubkey::new_rand();
