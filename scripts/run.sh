...
ok=true
for program in domichain-{faucet,genesis,keygen,validator}; do
  $program -V || ok=false
done
$ok || {
  echo
...
export RUST_LOG=${RUST_LOG:-domichain=info,domichain_runtime::message_processor=debug} # if RUST_LOG is unset, default to info
...
if [[ -e $validator_vote_account ]]; then
  ...
  domichain-keygen new --no-passphrase -so "$validator_vote_account"
fi
...
if [[ -e $validator_stake_account ]]; then
  ...
  domichain-keygen new --no-passphrase -so "$validator_stake_account"
fi
