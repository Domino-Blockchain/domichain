...
import { BlockResponse, PublicKey } from "@domichain/web3.js";
          ...
          <tbody>
            {block.rewards.map((reward, index) => {
              ...
              if (reward.postBalance !== null && reward.postBalance !== 0) {
                percentChange = (
                  (Math.abs(reward.satomis) /
                    (reward.postBalance - reward.satomis)) *
                  100
                ).toFixed(9);
              }
              return (
                <tr key={reward.pubkey + reward.rewardType}>
                  ...
                  <td>
                    <SolBalance satomis={reward.satomis} />
                  </td>
                  <td>
                    {reward.postBalance ? (
                      <SolBalance satomis={reward.postBalance} />
                    ) : (
                      "-"
                    )}
                  </td>
