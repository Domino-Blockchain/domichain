...
use {
    ...
    domichain_core::serve_repair::{RepairProtocol, RepairRequestHeader, ServeRepair},
    domichain_dos::cli::*,
    domichain_gossip::{
...
fn get_target(
    ...
    if nodes.is_empty() {
        // skip-gossip case
        target = Some((domichain_sdk::pubkey::new_rand(), entrypoint_addr));
    } else {
        info!("************ NODE ***********");
        for node in nodes {
            info!("{:?}", node);
...
pub mod test {
    use {
        ...
        domichain_gossip::contact_info::LegacyContactInfo,
